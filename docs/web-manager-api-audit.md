...
| `/api/v1/current-exits` | `GET` | None | JSON Array `[]CurrentExit`:<br>`slot` (int), `status` (string), `node_id` (string), `ip` (string), `country` (string), `region` (string), `score` (int), `reputation` (object), `network_class` (ASN/ISP/type and risk flags), `throughput` (int64 bps), `last_check` (RFC3339 string) | Bearer Token | Read-only | Low | **YES**: Dashboard Slot Overview cards; identifies active egress IPs, network ownership, and speeds. |
...
| `/api/v1/pool` | `GET` | None | JSON `PoolCounts`:<br>`active` (int), `standby` (int), `qualified` (int), `candidate` (int), `pending_review` (int), `cooldown` (int), `rejected` (int) | Bearer Token | Read-only | Low | **YES**: Dashboard Node Pool summary card. |
| `/api/v1/pool/qualified` | `GET` | None | JSON Array `[]models.Node`:<br>Only nodes in `REPUTATION_CHECKED`, `QUALIFIED`, or `STANDBY` status. Raw `DISCOVERED` nodes are never switch candidates. OpenVPN credentials stripped. Safe sanitized `.ovpn` config only. | Bearer Token | Read-only | Low | **YES**: Manual slot switch candidate selector with ASN and allocation-type filters. |
| `/api/v1/discovery/refresh` | `GET`, `POST` | None | GET returns current refresh status and last fetched/accepted/rejected counts. POST returns `202` and starts a serialized background VPN Gate refresh, reputation vetting, and three-slot capacity fill. | Bearer Token | POST mutates candidate pool | Medium | **YES**: Admin-only Pull Nodes & Fill Slots control. |
...
1. **Authentication:**
   - The daemon Agent API binds to the configured `api.listen` address with self-signed TLS and Bearer token enforcement. Multi-host Manager deployments normally use `0.0.0.0:60000` plus a firewall rule limited to the Manager source IP.
